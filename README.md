...
| מספר וואטסאפ, טלפון, מייל, אינסטגרם | `tools/site-content.js` → `contact` |
...
- **שאלות נפוצות** — נבנות מ-`tools/site-content.js`. **תשובה ריקה לא
  ...
  בתשובה אפשר לכתוב `{price}` ו-`{shipping}` במקום מספרים — הם נשאבים
  מהמחיר ומעלות המשלוח, כך שהשאלות לא יכולות לסתור את הדף. שדה
  `whatsapp: 'טקסט הקישור'` בשאלה מוסיף לה כפתור יצירת קשר בוואטסאפ.
- **פאנל "משלוחים והחזרות"** שנפתח בלחיצה, מעל השאלות הנפוצות. נבנה מ-
  `shipping` ב-`site-content.js`; כל עוד אין בו תוכן הוא לא מופיע בדף.
  `**טקסט**` בתחילת שורה הופך להדגשה, ו"דברו איתנו" מקשר ישירות לוואטסאפ.
  שדה `cost` כותב גם את שורת המחיר בפאנל וגם את `OfferShippingDetails`
  לגוגל — מספר אחד, שני מקומות, בלי סיכוי שייפרדו. `cost: 0` = משלוח חינם.
- **לכידת מיילים + שיתוף בוואטסאפ** בתחתית הדף.
- **כפתור וואטסאפ צף** בכל עמוד. מופיע אחרי 300 פיקסל גלילה כדי שלא
  יתחרה ב-CTA במסך הראשון, ונעלם כשפס יצירת הקשר על המסך. כשבאנר
  העוגיות פתוח הוא מתרומם מעליו לפי הגובה האמיתי שלו, שנמדד בזמן ריצה.
  כיבוי: `floatingButton: false` ב-`contact`.
- **פס יצירת קשר** בתחתית כל עמוד: וואטסאפ, טלפון (עם אייקון, כקישור
  `tel:`), מייל ואינסטגרם. **שדה ריק לא מוצג** — הדף לעולם לא מראה מספר
  טלפון שלא מצלצל.
- **כפתורי הרכישה פותחים וואטסאפ** עם ההזמנה כתובה מראש, כולל הכמות
  שנבחרה בדף. זה נדלק ברגע שיש `whatsappNumber`, ואפשר לכבות עם
  `ctaOpensWhatsapp: false`.
